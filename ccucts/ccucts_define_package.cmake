...
##
# documentation for ccucts_define_package function
#
...
#
# note: ccucts_define_package is improved create_ccutils_component function.
#       in ccucts_define_package there are no separate SOURCES/HEADERS parameters,
#       but instead ITEMS parameter.
#
# programmer should list items of package. items are essentially names of cpp/hpp files,
# but without extensions. ccucts_define_package take item name and then searches
...
# ccucts_define_package will try to find following files:
#   apple.cpp, apple.hpp,
...
#
# warning: current version of ccucts_define_package is not intended for creation of
...
function(ccucts_define_package)
  ...
  block()
    ...
    #   set(t_len 0)
    #   set(x "ccucts_define_package_arg_${i}")
    #   set(x1 "${x}")
    #   message(NOTICE "i: ${i}")
    #   message(NOTICE "x: ${x}")
    ...
    string(LENGTH "${ccucts_define_package_arg_PACKAGE_NAME}" t_len)
    if(t_len EQUAL 0)
  ...
  foreach(item ${ccucts_define_package_arg_ITEMS})
    ...
    set(filename_base_to_check
      "${item}"
      "${item}.any.any"
      "${item}.${ccucts_define_package_arg_CPU}.any"
      "${item}.any.${ccucts_define_package_arg_OS}"
  ...
  foreach(i ${found_sources})
    list(
      ...
      # "${CMAKE_CURRENT_SOURCE_DIR}/${ccucts_define_package_arg_SUBDIR}/${i}"
      "${i}"
    )
  endforeach()
  ...
  foreach(i ${ccucts_define_package_arg_HEADERS})
    list(
      APPEND
      formatted_headers_list
      "${ccucts_define_package_arg_SUBDIR}/includes/${ccucts_define_package_arg_DOMAIN_NAME}/${ccucts_define_package_arg_PACKAGE_NAME}/${ccucts_define_package_arg_SUBDIR}/${i}.hpp"
    )
  endforeach()
  ...
  target_sources(
    ${ccucts_define_package_arg_PACKAGE_NAME}
    PUBLIC
    FILE_SET ${ccucts_define_package_arg_PACKAGE_NAME}_headers
    TYPE HEADERS
    BASE_DIRS ${ccucts_define_package_arg_SUBDIR}/includes
    FILES
    ${formatted_headers_list}
  )
  ...
  target_link_libraries(
    ${ccucts_define_package_arg_PACKAGE_NAME}
    PUBLIC
    ${ccucts_define_package_arg_LIBS_PUB}
    PRIVATE
    ${ccucts_define_package_arg_LIBS_PRIV}
  )

  target_include_directories(
    ${ccucts_define_package_arg_PACKAGE_NAME}
    PUBLIC
    $<BUILD_INTERFACE:${CMAKE_CURRENT_SOURCE_DIR}/${ccucts_define_package_arg_SUBDIR}/includes>
    $<INSTALL_INTERFACE:${CMAKE_INSTALL_INCLUDEDIR}/${ccucts_define_package_arg_DOMAIN_NAME}/${ccucts_define_package_arg_PACKAGE_NAME}/${ccucts_define_package_arg_SUBDIR}>
    PUBLIC
    ${ccucts_define_package_arg_INC_DIRS_PUB}
    PRIVATE
  ...
  install(
    TARGETS ${ccucts_define_package_arg_PACKAGE_NAME}
    FILE_SET ${ccucts_define_package_arg_PACKAGE_NAME}_headers
    DESTINATION ${CMAKE_INSTALL_INCLUDEDIR}/${ccucts_define_package_arg_DOMAIN_NAME}/${ccucts_define_package_arg_PACKAGE_NAME}/${ccucts_define_package_arg_SUBDIR}
  )
